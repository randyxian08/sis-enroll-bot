...
## 静默调度器（推荐，sis-scheduler.py）

v13 之后的架构升级：**计时、对表、重试全部移到本地 Python**，页面只在收到开火命令的瞬间执行一串 fetch。

- 页面定时器在标签页后台会被 Chrome 钳制（1s 甚至 1min 粒度），本地定时器永不钳制——**标签页随便藏，全程零窗口打扰**；
- 每次开火命令自带全部逻辑（无状态注入），**页面被刷新也不丢引擎**；
- 开火链路纯 fetch 驱动（无 setTimeout），实测标签页 hidden 状态下整链 1.5 秒跑完。

```bash
python3 sis-scheduler.py sync      # 测对表
python3 sis-scheduler.py probe 0   # 干跑 Sem 1 链路（不点 Finish）
python3 sis-scheduler.py fire "2027-08-17T10:00:00+08:00" "2027-08-17T10:10:00+08:00" 15
# fire 第三个参数是重试窗口（分钟），建议 15
```

页面注入的 `enroll-bot.js`（含可视化面板）仍然保留，适合想盯着界面操作的人；`sis-scheduler.py` 适合挂上就走的无人值守场景。两者共用同一套表单重放协议。
...
1. 开抢前一天：更新开闸时间，`python3 sis-scheduler.py probe 0` 干跑验证链路；
2. 当天：提前 30 分钟登录（登录态一两天就过期），然后：

   ```bash
   nohup python3 sis-scheduler.py fire "T1" "T2" 15 > logs/fire.log 2>&1 &
   ```

   挂完该干嘛干嘛——标签页不用前台、不用面板、不用看门狗，全静默；
3. 服务器必崩，别慌——重试窗口 15 分钟起步，崩是计划内的事；
